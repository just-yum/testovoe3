...
import {Employee} from "../store/employeeSlice.ts";
...
export interface Response { success: boolean, error?: string }
export const loadEmployees = (): Promise<Employee[]> => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      if (!data) {
        reject({ success: false, error: 'Не удалось получить список пользователей'})
      }
...
export const addEmployee = (newEmployee: Omit<Employee, 'id'>): Promise<Response> => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      const newId = data[data.length - 1].id + 1;
      const q = {...newEmployee, id: newId}
      data = [...data, q];


      if (!data.find(el => el.id === newId)) {
        reject({ success: false, error: 'Не удалось добавить пользователя '})
      }
...
export const updateEmployee = (updatedEmployee: Employee): Promise<Response> => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      const index = data.findIndex(emp => emp.id === updatedEmployee.id);

      if (index === -1) {
        reject({ success: false, error: "Не удалось обновить данные о пользователе" });
      }
